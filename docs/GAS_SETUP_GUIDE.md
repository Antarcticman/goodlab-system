...
5. 查看下方執行紀錄，並確認 `f10943138@ntu.edu.tw` 收到「GAS 連線與寄信成功」。

測試信會列出 members、logs、routines 的實際筆數。請與網站資料大致核對；如果筆數明顯太少，先不要裝觸發器。

如要查看正式值日提醒的內容與排版，可再執行 `testDutyReminderToMe`。它會套用本週值日生資料，但只把預覽信寄到 `f10943138@ntu.edu.tw`，不會寄給真正的值日生。

如要查看提交完成後寄給全體在學成員的摘要，可執行 `testDutyCompletionToMe`。預覽同樣只寄到 `f10943138@ntu.edu.tw`，且不會寫入正式通知的防重寄標記。
